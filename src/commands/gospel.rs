...
pub fn gospel(channel: &str, display_name: &str) -> Option<String> {
    ...
    if !channel.eq_ignore_ascii_case(display_name) {
        config = Config::load(channel);
    }
    config.add_channel_metrics_gospel_english();

    Some(command_success)
}

pub fn evangelio(channel: &str, display_name: &str) -> Option<String> {
    ...
    config.add_channel_metrics_gospel_spanish();

    Some(command_success)
}

pub fn evangelium(channel: &str, display_name: &str) -> Option<String> {
    ...
    config.add_channel_metrics_gospel_german();
